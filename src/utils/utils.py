...
@torch.no_grad()
def calc_precision_and_racall(pred_y, y):
    """Calculate precision."""
    mask1 = pred_y == 1
    mask2 = pred_y == y
    positive_pred = np.sum(mask1)
    positive_case = np.sum(y)
    tp = np.sum(mask2 & mask1)
    fp = positive_pred - tp
    fn = positive_case - tp
    precision = tp / (tp + fp + 1e-6)
    recall = tp / (tp + fn + 1e-6)
    # fp = ((not mask2) & mask1).sum()
